    ...
    <View style={[styles.root, { backgroundColor: SAHEL.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: insets.bottom + 120 }}>
        <View style={[styles.heroWrap, { paddingTop: topPad }]}>
          <LinearGradient colors={['#7C3AED', SAHEL.primary]} style={styles.hero}>
        ...
        <View style={styles.infoRow}>
          <View style={styles.infoItem}>
            <Ionicons name="calendar-outline" size={18} color={SAHEL.accent} />
          ...
          <View style={styles.ticketList}>
            {m.ticket_types.map((ticket, i) => (
              <Pressable key={i}
                ...
                {selectedTicket === i && <Ionicons name="checkmark-circle" size={20} color={SAHEL.accent} />}
              </Pressable>
            ))}
          </View>
        </View>
        ...
        {selectedTicket !== null && (
          <View style={styles.section}>
            ...
            <View style={styles.qtyRow}>
              <Pressable style={styles.qtyBtn} onPress={() => setTicketQty(Math.max(1, ticketQty - 1))}><Ionicons name="remove" size={18} color={SAHEL.primary} /></Pressable>
              <Text style={styles.qtyCount}>{ticketQty}</Text>
              <Pressable style={styles.qtyBtn} onPress={() => setTicketQty(Math.min(10, ticketQty + 1))}><Ionicons name="add" size={18} color={SAHEL.primary} /></Pressable>
            </View>
          </View>
        )}
      ...
      <View style={[styles.bottomBar, { paddingBottom: insets.bottom + 12 }]}>
        ...
        <Pressable style={[styles.bookBtn, selectedTicket === null && { opacity: 0.5 }]}
          onPress={() => { if (selectedTicket === null) { showToast('Select a ticket type', 'info'); return; } hapticSuccess(); showToast('Tickets reserved! Check your email.', 'success'); }}>
...
const styles = StyleSheet.create({
  ...
  ticketList: { gap: 10 },
  ticketCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderRadius: 14, borderWidth: 1.5, borderColor: SAHEL.border, padding: 16 },
  ticketSelected: { borderColor: SAHEL.accent, backgroundColor: '#F0FDFA' },
  ticketLeft: { flex: 1 },
  ticketName: { fontSize: 15, fontFamily: 'mon-b', color: SAHEL.dark },
  ticketAvail: { fontSize: 12, fontFamily: 'mon', color: '#94A3B8', marginTop: 2 },
  ticketPrice: { fontSize: 16, fontFamily: 'mon-b', color: SAHEL.primary, marginRight: 10 },
  qtyRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 20, backgroundColor: '#fff', borderRadius: 14, borderWidth: 1, borderColor: SAHEL.border, padding: 16 },
  qtyBtn: { width: 40, height: 40, borderRadius: 20, borderWidth: 1.5, borderColor: SAHEL.border, alignItems: 'center', justifyContent: 'center' },
  qtyCount: { fontSize: 22, fontFamily: 'mon-b', color: SAHEL.dark, minWidth: 30, textAlign: 'center' },
