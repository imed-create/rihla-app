    ...
    return (
      <View style={[styles.root, { paddingTop: topPad + 40 }]}>
        ...
        <View style={styles.notFound}>
          ...
          <Text style={[styles.notFoundText, { color: colors.muted }]}>Destination not found</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.bg }]}>
      ...
      <ScrollView
        ...
        contentContainerStyle={[styles.filterScroll, { backgroundColor: colors.card, borderBottomColor: colors.border }]}
      >
        <Pressable
          style={[styles.filterPill, { borderColor: colors.border, backgroundColor: colors.card }, selectedCategory === 'all' && styles.filterPillActive]}
          onPress={() => { hapticLight(); setSelectedCategory('all'); }}
        >
          <Ionicons name="grid-outline" size={14} color={selectedCategory === 'all' ? '#fff' : colors.muted} />
          <Text style={[styles.filterPillText, { color: colors.muted }, selectedCategory === 'all' && { color: '#fff' }]}>All</Text>
        </Pressable>
        {MARKETPLACE_CATEGORIES.map((cat) => {
          ...
          return (
            <Pressable
              key={cat.key}
              style={[styles.filterPill, { borderColor: colors.border, backgroundColor: colors.card }, isActive && { backgroundColor: cat.color, borderColor: cat.color }]}
            ...
            >
              ...
              <Text style={[styles.filterPillText, { color: colors.muted }, isActive && { color: '#fff' }]}>{cat.label}</Text>
              {count > 0 && (
                <View style={[styles.filterPillCount, { backgroundColor: colors.border }, isActive && { backgroundColor: 'rgba(255,255,255,0.25)' }]}>
                  <Text style={[styles.filterPillCountText, { color: colors.muted }, isActive && { color: 'rgba(255,255,255,0.8)' }]}>{count}</Text>
                </View>
              )}
            </Pressable>
          );
      ...
      <View style={[styles.listingsSheet, { backgroundColor: colors.card, borderTopColor: colors.border }]}>
        <View style={styles.sheetHandle} />
        <View style={styles.sheetHeader}>
          <Text style={[styles.sheetTitle, { color: colors.text }]}>
            {selectedCategory === 'all' ? 'All Listings' : getCategoryDef(selectedCategory).labelPlural}
          </Text>
          <Text style={[styles.sheetCount, { color: colors.muted }]}>{listings.length} available</Text>
        </View>
        <ScrollView
        ...
        >
          ...
          {listings.length === 0 && (
            <View style={styles.empty}>
              ...
              <Text style={[styles.emptyText, { color: colors.muted }]}>No listings in this category</Text>
            </View>
          )}
        </ScrollView>
      </View>
      ...
      <View style={styles.listingInfo}>
        ...
        <View style={styles.listingBottom}>
          <View style={styles.listingRating}>
            ...
            <Text style={[styles.listingRatingText, { color: colors.text }]}>{item.rating}</Text>
            <Text style={[styles.listingReviewCount, { color: colors.muted }]}>({item.review_count})</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  // Filter pills
  filterScroll: { paddingHorizontal: 12, paddingVertical: 10, gap: 8, borderBottomWidth: 1 },
  filterPill: {
  ...
  filterPillText: { fontSize: 12, fontFamily: 'mon-sb' },
  filterPillCount: { paddingHorizontal: 5, paddingVertical: 1, borderRadius: 8, marginLeft: 2 },
  filterPillCountText: { fontSize: 10, fontFamily: 'mon-b' },
  ...
  sheetTitle: { fontSize: 16, fontFamily: 'mon-b' },
  sheetCount: { fontSize: 12, fontFamily: 'mon-sb' },
  ...
  listingFeatBadge: { flexDirection: 'row', alignItems: 'center', gap: 2, paddingHorizontal: 5, paddingVertical: 1, borderRadius: 4, backgroundColor: RIHLA.primary + '20' },
  listingFeatText: { fontSize: 8, fontFamily: 'mon-b', color: RIHLA.primary },
  listingTitle: { fontSize: 14, fontFamily: 'mon-b' },
  ...
  listingRatingText: { fontSize: 12, fontFamily: 'mon-b' },
  listingReviewCount: { fontSize: 10, fontFamily: 'mon' },
  listingPrice: { fontSize: 13, fontFamily: 'mon-b', color: RIHLA.primary },

  // Empty
  empty: { alignItems: 'center', paddingVertical: 40, gap: 8 },
  emptyText: { fontSize: 14, fontFamily: 'mon-sb' },
});
