...
export default function BeachItemsScreen() {
  ...
  const handleBook = async () => {
    ...
    addBooking({
      ...
      subtitle: selectedPrice.label,
      price: selectedPrice.price,
      details: { itemId: selectedItem.id, duration: selectedPrice.label },
    });
    setLoading(false);
    setSuccess(true);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <Text style={styles.headerSub}>Jet Ski · Pedalo · Water Fun</Text>
      </LinearGradient>
      ...
      {success ? (
        <View style={styles.successWrap}>
          ...
          <Text style={[styles.successSub, { color: colors.mutedForeground }]}>
            Head to the water sports booth to get started
          </Text>
        </View>
      ) : (
        <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: insets.bottom + 140, gap: 20 }} showsVerticalScrollIndicator={false}>
            ...
            {ITEMS.map((item) => (
              ...
              >
                ...
                <View style={styles.availRow}>
                  ...
                  <Text style={{ color: "#06D6A0", fontSize: 12, fontFamily: "Inter_500Medium" }}>
          ...
          <View style={styles.priceRow}>
            {selectedItem.prices.map((p) => (
              ...
              >
                <Text style={[styles.priceDuration, { color: selectedPrice.label === p.label ? "#FFF" : colors.foreground }]}>
                  {p.label}
                </Text>
                <Text style={[styles.priceAmount, { color: selectedPrice.label === p.label ? "rgba(255,255,255,0.9)" : colors.primary }]}>
                  {p.price} DZD
                </Text>
              </Pressable>
            ))}
          ...
          <View style={[styles.safetyCard, { backgroundColor: "#FFF8E1", borderColor: "#C9A84C" }]}>
            ...
            <Text style={{ flex: 1, color: "#9A7A20", fontSize: 13, fontFamily: "Inter_400Regular", lineHeight: 18 }}>
      ...
      {!success && (
        <View style={[styles.footer, { paddingBottom: insets.bottom + 16, backgroundColor: colors.background }]}>
          <ConfirmButton label={`Book ${selectedItem.name}`} onPress={handleBook} loading={loading} price={selectedPrice.price} />
        </View>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  itemName: { fontSize: 16, fontFamily: "Inter_700Bold", textAlign: "center" },
  itemDesc: { fontSize: 12, fontFamily: "Inter_400Regular", textAlign: "center", lineHeight: 16 },
  availRow: { flexDirection: "row", alignItems: "center" },
  priceRow: { flexDirection: "row", gap: 10 },
  priceCard: { flex: 1, borderRadius: 14, padding: 14, alignItems: "center", gap: 4 },
  priceDuration: { fontSize: 14, fontFamily: "Inter_600SemiBold" },
  priceAmount: { fontSize: 13, fontFamily: "Inter_700Bold" },
