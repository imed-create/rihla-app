...
export default function ActivityDetailScreen() {
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    slotList: { gap: 8 },
    slotCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.card, borderRadius: 12, borderWidth: 1, borderColor: colors.border, padding: 14 },
    slotFull: { opacity: 0.5 },
    slotSelected: { borderColor: RIHLA.accent, backgroundColor: colors.card },
    slotLeft: { flex: 1 },
    slotDate: { fontSize: 13, fontFamily: 'mon-b', color: colors.text },
    slotTime: { fontSize: 12, fontFamily: 'mon', color: colors.muted, marginTop: 2 },
    slotSpots: { fontSize: 12, fontFamily: 'mon-sb', color: RIHLA.accent, marginRight: 8 },
        ...
        <View style={styles.section}>
          ...
          <View style={styles.guestRow}>
            ...
            <View style={styles.guestControls}>
              ...
              <Pressable style={styles.guestBtn} onPress={() => setParticipants(Math.min(m.max_participants, participants + 1))}><Ionicons name="add" size={18} color={RIHLA.primary} /></Pressable>
            </View>
          </View>
        </View>
          ...
          <View style={styles.slotList}>
            {MOCK_SCHEDULES.map((slot) => (
              <Pressable key={slot.id}
                ...
                {selectedSlot === slot.id && <Ionicons name="checkmark-circle" size={20} color={RIHLA.accent} />}
              </Pressable>
            ))}
          </View>
        </View>
